...
namespace JJ.Framework.Mvc
{
    ...
    {
        public static HtmlString ActionLinkWithCollection<T>(this HtmlHelper htmlHelper, string linkText, string actionName, string controllerName, string parameterName, IEnumerable<T> collection)
        {
        ...
        public static IHtmlContent ActionLinkWithParams(this HtmlHelper htmlHelper, string actionName, string controllerName, params object[] parameterNamesAndValues)
        {
        ...
        public static HtmlString HiddenForAllProperties(this HtmlHelper htmlHelper, object model)
        {
            if (model == null)
            {
...
- In the Get action method, pick up the viewmodel out of the TempData dictionary if it is present there. Otherwise just create new one.")]
        public static HtmlString HiddenFor<T>(this HtmlHelper htmlHelper, Expression<Func<T>> expression)
        {
            //string name = JJ.Framework.Reflection.ExpressionHelper.GetName(expression);
            string name = Microsoft.AspNetCore.Mvc.ExpressionHelper.GetExpressionText(expression); // The one from MVC will get the appropriate portion of a complex expression, for when you don't use BeginCollection().
...
- Then do a RedirectToAction to the Get action.
- In the Get action method, pick up the viewmodel out of the TempData dictionary if it is present there. Otherwise just create new one.")]
        public static HtmlString Hidden(this HtmlHelper htmlHelper, string name, object value)
        {
